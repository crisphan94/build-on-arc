...
export async function agentPay(
  ...
  // Probe: expect 402 with payment requirements
  ...
  // Parse payment requirements from body or header
  ...
  // Find the GatewayWalletBatched option for Arc Testnet
  ...
  if (!option)
    throw new Error('No GatewayWalletBatched option for Arc Testnet in payment requirements')

  const verifyingContract = option.extra!.verifyingContract as `0x${string}`
  const paymentAmount = BigInt(option.amount)

  // Ensure agent has sufficient gateway balance (auto-deposit if needed)
  console.log('[agent-pay] Ensuring gateway balance for payment:', paymentAmount.toString())
  try {
    await ensureGatewayBalance(paymentAmount)
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    throw new Error(`Gateway deposit failed: ${msg}`)
  }

  // Sign EIP-3009 TransferWithAuthorization via Circle Wallets (or raw key fallback)
  ...
  const paidRes = await fetch(serviceUrl, { headers: { 'Payment-Signature': paymentHeader } })
  if (!paidRes.ok) {
  ...
  return { data, payer: agentAddress, amount: option.amount, formattedAmount, service: serviceId }
}
